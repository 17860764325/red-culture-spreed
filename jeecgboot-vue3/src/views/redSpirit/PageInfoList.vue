<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <PageInfoModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import {pageInfoColumns} from './RedSpirit.data';
  import {pageInfoList, pageInfoDelete, pageInfoDeleteBatch, pageInfoExportXlsUrl, pageInfoImportUrl } from './RedSpirit.api';
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        ...
        importConfig: {
            url: ()=>{
                return pageInfoImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    watch(mainId, () => {
         searchInfo['pageNumber'] = unref(mainId);
         reload();
      }
    );
    ...
    async function handleDelete(record) {
        await pageInfoDelete({id: record.id}, handleSuccess);
    }
    ...
    async function batchHandleDelete() {
        await pageInfoDeleteBatch({ids: selectedRowKeys.value}, handleSuccess)
    }
