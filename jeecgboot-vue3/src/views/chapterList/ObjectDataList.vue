<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <ObjectDataModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import ObjectDataModal from './components/ObjectDataModal.vue'
  import {objectDataColumns} from './ChapterList.data';
  import {objectDataList, objectDataDelete, objectDataDeleteBatch, objectDataExportXlsUrl, objectDataImportUrl } from './ChapterList.api';
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        ...
        importConfig: {
            url: ()=>{
                return objectDataImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    watch(mainId, () => {
         searchInfo['chapterCode'] = unref(mainId);
         reload();
      }
    );
    ...
    async function handleDelete(record) {
        await objectDataDelete({id: record.id}, handleSuccess);
    }
    ...
    async function batchHandleDelete() {
        await objectDataDeleteBatch({ids: selectedRowKeys.value}, handleSuccess)
    }
